...
import { User as UserIcon, Package, MapPin, Lock, LogOut, Edit2 } from 'lucide-react'
  ...
  return (
    <div className="container py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Quản lý thông tin cá nhân và đơn hàng</p>
      </div>
      ...
      <Tabs defaultValue="profile" className="space-y-6">
        ...
        <TabsList className="grid w-full grid-cols-2 lg:grid-cols-4 h-auto gap-2 bg-transparent p-0">
          <TabsTrigger
            value="profile"
            className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground bg-card border border-border rounded-xl py-3 gap-2"
          >
              ...
              <div className="flex items-center gap-4">
                <div>
                  ...
                  <p className="text-muted-foreground text-sm">
                    Thành viên từ {formattedJoinedDate}
                  </p>
                </div>
              </div>

              <Separator />
              <form onSubmit={submitProfile(onSubmitProfile)} className="space-y-6">
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    <Label className="text-foreground mb-2">Họ tên</Label>
                    <Input {...registerProfile('name', { required: true })} disabled={!isEditing} />
                  </div>

                  <div>
                    <Label className="text-foreground mb-2">Email</Label>
                    <Input {...registerProfile('email')} disabled={!isEditing} />
                  </div>

                  <div>
                    <Label className="text-foreground mb-2">Số điện thoại</Label>
                    <Input {...registerProfile('phone')} disabled={!isEditing} />
                  </div>

                  <div>
                    <Label className="text-foreground mb-2">Ngày sinh</Label>
                    <Input type="date" {...registerProfile('dob')} disabled={!isEditing} />
                  </div>
                </div>
                      ...
                      {order.items.map((item, index) => (
                        <div key={index} className="flex justify-between text-sm">
                          <span className="text-muted-foreground">
                            {item.productName} {item.variantName ? `(${item.variantName})` : ''} x
                            {item.quantity}
                          </span>
        ...
        <TabsContent value="security">
          <Card>
            ...
            <CardContent className="space-y-6">
              <form onSubmit={submitPassword(onSubmitPassword)} className="space-y-4 max-w-md">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    {...registerPassword('passwordConfirm', {
                      validate: (v) => v === newPassword.current || 'Mật khẩu không khớp',
                    })}
                  />
                </div>
